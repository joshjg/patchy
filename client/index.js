...
const socket = process.env.NODE_ENV === 'development' ? io.connect('http://localhost:8000') : io.connect('198.46.131.235:3000');
...
const store = createStore(
  ...
  process.env.NODE_ENV === 'development' ?
    compose(applyMiddleware(userIdMiddleware, socketMiddleware(socket), localUserMiddleware), window.devToolsExtension && window.devToolsExtension())
    : applyMiddleware(userIdMiddleware, socketMiddleware(socket), localUserMiddleware)
);
